...
function Form({ priceInCents, productId } : { priceInCents : number, productId: string})
{
    ...
    async function handleSubmit(e : FormEvent) // handles everything regarding strip
    {
        e.preventDefault() // prevent the default behavior of form etc

        // cannot submit form if these entities are null
        if(stripe == null || elements == null || email == null){return}
        setIsLoading(true)

        // check for existing order.. cant purchase 2x
        const orderExists = await userOrderExists(email, productId)
        if(orderExists)
            {
                setErrorMessage(`Purchase already exists: -> Try downloading from My Orders Page`)
                setIsLoading(false)
                return
            }
        ...
        stripe.confirmPayment({
            ...
            }).finally(() => setIsLoading(false)) // so you can resubmit 
    }
